  ...
  useEffect(() => {
    if (cart.length > 0 && cart[0]?.cart?.items) {
     ...
     const handleaddtocart = async(id ,qty)=>{
      ...
      console.log("this is cart",cart[0]?.cart?.items?.length)
  return (
    <>
                    ...
                    <div className="flex gap-10 flex-col md:flex-row">
                      <div className="w-[60%] flex gap-10">
                        ...
                        <div className="border border-black text-black lg:px-3 rounded-2xl flex items-center gap-2 lg:gap-4 h-fit lg:py-2 p-2 w-fit">
                          Qty:<h1 className="text-xl cursor-pointer" onClick={()=>handledecreseqty(item.productid._id )}>-</h1>
                        ...
                        <h1>Price:₹{item.productid.offerPrice * item.quantity}</h1>
                      </div>
      ...
      </div>:<div className="flex justify-center items-center text-2xl text-red-500 py-5"><h1>there is no product in your cart </h1></div>
      }
      <RelatedProducts/>
      <Feature/>
      <Footer />
