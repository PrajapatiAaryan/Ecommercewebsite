...
const DetailedProduct = () => {
  ...
  useEffect(() => {
      dispatch(getdetailproduct()); // Fetch products when the component mounts
    }, [dispatch]);
  ...
  // console.log("detailed product" , detailproduct)  detailed product 
  ...
  return (
    <>
      ...
      <div className="w-full  p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-4">
                ...
                <p className="text-gray-700 leading-relaxed">
                  {detailproduct.shortDescription} Lorem ipsum dolor sit amet consectetur
                  adipisicing elit.
                </p>
                ...
                <div>
                  ...
                  <div className="flex gap-2 pt-2">
                    {detailproduct?.availableSizes?.map((size, idx) => (
                      <div
                        key={idx}
                        className="w-10 h-10 flex items-center justify-center border-2 border-gray-300 rounded-md text-lg font-semibold cursor-pointer hover:border-black"
                      >
                        {size}
                      </div>
                    ))}
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center border border-gray-400 px-4 py-2 rounded-md gap-4 text-xl">
                    <button className="text-gray-600 hover:text-black cursor-pointer" onClick={()=>setqty((prev)=>prev<=1?1:prev-1)}>-</button>
                    <input type="text" value={qty} onChange={(e)=>handlechangeqty(e.target.value)} className="w-8 outline-none pl-3"/>
                    <button className="text-gray-600 hover:text-black cursor-pointer" onClick={()=>setqty((prev)=>prev+1)}>+</button>
                  </div>
            ...
            {/* Tabs Section */}
            <div className="mt-10">
              <div className="flex gap-6 border-b border-gray-300 text-lg">
